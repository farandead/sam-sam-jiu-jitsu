  ...
  return (
    <PageLayout className="pt-10">
      <div className="max-w-4xl mx-auto p-6">
        <h1 className="text-4xl font-bold text-center mb-8 text-[#23b6cb]">
          Class Timetable
        </h1>
        <p className="text-gray-600 mb-8 text-center">
          These are the classes we are offering at the moment
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {classSchedule.map((schedule, index) => (
            ...
            >
              ...
              {schedule.classes.map((classInfo, classIndex) => (
                <div key={classIndex} className="mb-4">
                  <p className="text-lg font-medium mb-2">{classInfo.className}</p>
                  {classInfo.times.map((time, timeIndex) => (
                    <span
                      key={timeIndex}
