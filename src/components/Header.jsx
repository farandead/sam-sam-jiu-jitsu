    ...
    return (
        <div className="sticky top-0 left-0 w-full z-50 bg-white border-b shadow-md">
            <div className="flex items-center justify-between px-5 lg:px-7.5 xl:px-0 py-4">
                ...
                {/* Hamburger Button */}
                <button
                    className={`lg:hidden px-3 z-50 ${openNavigation ? 'fixed top-4 right-4' : 'ml-auto'}`}
                ...
                >
                    <div className={`relative z-2 flex flex-col items-center justify-center w-full h-screen lg:h-auto lg:flex-row ${openNavigation ? 'lg:justify-center' : ''}`}>
                        {navigation.map((item) => (
                            <a
                                key={item.id}
                                href={item.url}
                                ...
                                className={`block relative font-code text-2xl capitalize text-black transition-colors hover:text-[#23b6cb] ${
                                    ...
                                    item.url === location.hash ? 'z-2 lg:text-black' : 'lg:text-black'
